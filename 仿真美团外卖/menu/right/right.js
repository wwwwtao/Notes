(function(){
    ...
    function initRightList(list){
        ...
        list.forEach(function(item, index){
            ...
            var $target = $(str);  //.menu-item

            $target.data('itemData',item);  //把item 挂载到 .menu-item上 数据名称 itemData
    ...
    function addClick(){
        $('.menu-item').on('click', '.plus', function(e){
            var $count = $(e.currentTarget).parent().find('.count');  //find 查找后代中 的 类名选择器.count 的元素

            $count.text(parseInt($count.text()||'0')+1);                // 数量加1

            var $item = $(e.currentTarget).parents('.menu-item').first();    //parents方法 元素是按照从最近的父元素向外的顺序被返回的

            var itemData = $item.data('itemData');  //取到 .menu-item 的 itemData数据  也就是item
